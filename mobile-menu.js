...
(function() {
  ...
  hamburger.setAttribute('aria-label', isFr ? 'Ouvrir le menu' : 'Open menu');
  ...
  overlay.setAttribute('aria-label', isFr ? 'Navigation' : 'Navigation');
  ...
  if (navActions) {
    ...
    const booking = navActions.querySelector('.demo-booking');
    if (login) menuFooter.appendChild(login.cloneNode(true));
    if (booking) {
      const clone = booking.cloneNode(true);
      clone.querySelector('.demo-booking-popover').id = 'mobile-demo-options';
      clone.querySelector('[data-book-demo]').setAttribute('aria-controls', 'mobile-demo-options');
      menuFooter.appendChild(clone);
    }
  }
  ...
  function close() {
    ...
    overlay.inert = true;
    hamburger.setAttribute('aria-expanded', 'false');
    overlay.querySelectorAll('[data-book-demo]').forEach(button => button.setAttribute('aria-expanded', 'false'));
    overlay.querySelectorAll('.demo-booking-popover').forEach(panel => { panel.hidden = true; });
    hamburger.focus();
  }
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Tab') {
      const focusable = [...overlay.querySelectorAll('a[href], button')].filter(el => el.getClientRects().length);
...
/** Right-aligned booking disclosures, including the mobile-menu clone. */
(function() {
  document.querySelectorAll('.demo-booking').forEach(function(booking) {
    const trigger = booking.querySelector('[data-book-demo]');
    const panel = booking.querySelector('.demo-booking-popover');
    ...
    booking.addEventListener('keydown', function(event) {
    ...
    document.addEventListener('click', function(event) {
      if (!booking.contains(event.target)) close(false);
    });
    booking.addEventListener('focusout', function(event) {
      if (!booking.contains(event.relatedTarget)) close(false);
    });
...
(function() {
  ...
  const copy = isFr ? {
    ...
    items: [
      ...
      ['What information do I get?', "The executive's first and last name, mobile number (and landline if provided), email and LinkedIn profile. You also get the reason for the call: the change in situation we detected, with its date."],
