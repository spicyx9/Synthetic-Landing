...
The following section uses six selectable company records instead of a text grid. Each illustrative record shows a before/after change, the green scan treatment and a reason to contact the company. `js/pages/home-signals.js` advances once through the six examples while the record is in view; clicking a selector keeps that example open. Clicking the record advances to the next example, wrapping after the last; each newly selected example replays the scan. Keyboard focus pauses progression, hidden/offscreen pages preserve progress, and reduced motion leaves a static manually selectable record. No external business data is requested.
