...
for(const file of ['index.html','index-fr.html'])test(`${file}: compact conversion flow and honest integration states`,()=>{
 ...
 assert.equal((html.match(/class="prospect-detail-area"/g)||[]).length,1);
 ...
 assert.doesNotMatch(output,/href="(?:tel:|mailto:)/);
 assert.match(output,/href="https:\/\/www.linkedin.com\/"/);
});
