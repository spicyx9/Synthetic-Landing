window.SwarmMotion?.ready(M => {
  ...
  document.querySelectorAll('.pricing-section').forEach(root => {
    ...
    add('.pricing-range-label,.pricing-range',340);
    add('.pricing-range-steps button',430,'fade');
    add('.pricing-config-price,.pricing-config-leads',640,'fade');
    add('.pricing-config-features li',720,'fade');
    add('[data-pricing-checkout]',1000);
  ...
  document.querySelector('[data-pricing-range]')?.addEventListener('input',animatePrice);
  document.querySelectorAll('[data-pricing-step]').forEach(button=>button.addEventListener('click',animatePrice));
