...
| --- | --- |
| `styles.css` | Global reset, Inter body, sticky header, base buttons, hero/aurora, mobile menu and booking; also substantial older landing-page/widget rules. Loaded on all public pages. |
| `site-pages.css` | Current corporate pages, common cards, footer, FAQ, focus treatment, language selector, header/mobile overrides. Loaded on all public pages. |
| `home.css` | Scoped `.home` tokens, hero/news, current before/after section and homepage spacing; some older `.hs-*` patterns remain. |
...
Primary stack, from `styles.css` body:
...
Reduced motion disables aurora/decor animation and selected transitions in `site-pages.css`. Older isolated keyframes remain in base/demo CSS; a comprehensive reduced-motion audit has not been performed. Light gray text and bright semantic colors are not automatically contrast-compliant: check their actual size/background before reuse.
