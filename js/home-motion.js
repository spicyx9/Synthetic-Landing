window.SwarmMotion?.ready(M => {
  ...
  const syncTitleScan=()=>hero.classList.toggle('hero-scan-active',titleVisible&&!document.hidden&&!M.reduced());
  if('IntersectionObserver' in window){
    const titleObserver=new IntersectionObserver(entries=>{
      titleVisible=entries[0].isIntersecting;
      syncTitleScan();
    },{threshold:0});
    titleObserver.observe(q('.hero-word-highlight'));
    document.addEventListener('visibilitychange',syncTitleScan);
  }
  M.sequence(hero, [
